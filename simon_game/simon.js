...
function init() {
	...
	const strictPop = document.querySelector('#strictSuccess');
	//variables
	let sequence = [];
	let aiPlay;
	...
	//create new game after win
	function newGame() { //console.log('newgame')
		win = false;
		sequence = [];
		clear();
		endPop.style.display = 'none';
		strictPop.style.display = 'none';		
		const nextGame = computerSequence(20, 0, 3);
		setTimeout(()=> playSequence(nextGame), 1500);
	}
	...
	start.addEventListener('click', ()=> { 
		...
		//strictPop.style.display = 'none'; 	
		if(gameOn) {
			if(!normalMode && !strict && !win) { //if 1st game
				normalMode = true;
				setTimeout(()=> playSequence(sequence), 1000);
			}else if(!win && normalMode || !win && strict) { //if player click again on start: shuffle and start again
				clearInterval(aiPlay); 
				clear(); 
				let shuffled = shuffle(sequence);
				setTimeout(()=> playSequence(shuffled), 1500);
			}else if(win) {
				clearInterval(aiPlay);
				newGame();
			}
