...
function init() {
	...
	start.addEventListener('click', ()=> { 
		if(!normalMode && !strict) { //if 1st game
			endPop.style.display = 'none';
			normalMode = true;
			setTimeout(()=> playSequence(sequence), 1000);
		}else if(normalMode && !strict) { //if player click again on start: shuffle and start again
			setTimeout(()=> clear());
			let shuffled = shuffle(sequence);
			setTimeout(()=> playSequence(shuffled), 1000);
		}else if(strict && !normalMode) { //if player is on strict mode and wants to play normal
			strict = false;
			normalMode = true;
			clear();
			let shuffled = shuffle(sequence);
	...
	strictBtn.addEventListener('click', ()=> {console.log('strict', strict)
		if(!normalMode && !strict) {
			strictMode(); 
		}else if(normalMode && !strict) { //if player wants to change mode while normal mode
			normalMode = false;
			strict = true;
			sequence = [];
			clear();
			newStrict();
		}else if(strict && !normalMode) {//if player click again on strict while playing strict
			console.log('strict again')
			clear();
			sequence = [];
			newStrict();
		}
